...
def main():
    ...
    insert_traders = [
        one_trader.strip() % dict(tid=n, name=str(n)) for n in range(100, 112)
    ]
    if lob_connection:
    ...
    print(
        "Large incoming bid limit crosses best ask.\
    ...
    print(
        "A market order takes the specified volume from the\
    ...
    modifyOrder5 = {"side": "bid", "qty": 14, "price": 99, "tid": 100}
    lob.modifyOrder(5, modifyOrder5)
    print("book after increase amount. will be put as end of queue")

    lob.print(instrument)

    modifyOrder5 = {"side": "bid", "qty": 14, "price": 103.2, "tid": 100}
    lob.modifyOrder(5, modifyOrder5)
    ...
    trades, idNum = lob.processOrder(marketOrder, False, False)
    print(
        "A market ask for 40 should take all the bids and keep the remainder in the book",
        marketOrder,
    )
    lob.print(instrument)
