...
class OrderBook(object):
    ...
    def __init__(self, db, tick_size=0.0001):
        ...
        self.best_quotes_order_desc = self.best_quotes_order.replace(
            ":direction", "desc"
        )
        self.best_quotes_order_map = dict(
            desc=self.best_quotes_order_desc, asc=self.best_quotes_order_asc
        )
    ...
    def processMatchesDB(self, quote, crsr, verbose):
        ...
        for match in matches:
            ...
            if verbose:
                print(
                    ">>> TRADE \nt=%s $%f n=%d p1=%d p2=%d"
                    % (self.time, price, qty, counterparty, quote["tid"])
                )
        if trades:
            crsr.executemany(self.insert_trade, trades)
            self.lastPrice[instrument] = price
            crsr.execute(
                self.set_lastprice, dict(instrument=instrument, lastprice=price)
            )
        return trades, quote
    ...
    def getPrice(self, instrument, side, direction="asc"):
        ...
        first = crsr.execute(
            sql_active_orders, dict(instrument=instrument, side=side)
        ).fetchone()
        if first:
    ...
    def print(self, instrument):
        crsr = self.db.cursor()
        sql_active_orders = self.active_orders + self.best_quotes_order_asc

        fileStr = StringIO()
        fileStr.write("------ Bids -------\n")
        for bid in crsr.execute(
            sql_active_orders, dict(instrument=instrument, side="bid")
        ):
        ...
        fileStr.write("\n------ Asks -------\n")
        for ask in crsr.execute(
            sql_active_orders, dict(instrument=instrument, side="ask")
        ):
        ...
        fileStr.write(
            "volume bid if i ask 98: "
            + str(self.getVolumeAtPrice(instrument, "bid", 98))
            + "\n"
        )
        fileStr.write(
            "volume ask if i bid 101: "
            + str(self.getVolumeAtPrice(instrument, "ask", 101))
            + "\n"
        )
